...
func InitStateMachine(id uint64, peers []uint64, majority uint64, electionTimeout, heartbeatTimeout time.Duration, currentTerm, votedFor uint64, log *log.Log) *StateMachine {
	...
	sm.alarmChan = make(chan Action, 1000)
	sm.commitChan = make(chan Action, 1000)
	sm.saveChan = make(chan Action, 1000)
	sm.sendChan = make(chan Action, 1000)

	sm.timeoutChan  = make(chan Event, 1000)
	sm.responseChan  = make(chan Event, 1000)
	sm.requestChan   = make(chan Event, 1000)
